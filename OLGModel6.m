%% OLG Model 6: Stochastic OLG Model
% Add idiosyncratic shocks, so we now have an incomplete markets OLG model.
% We introduce z and e. z is a (log) AR(1) process with age-dependent parameters, which is
% an idiosyncratic shock to 'hourly-earnings' (technically, earnings per
% unit of time, our model does not contain a concept of 'hours'). e is an
% iid (log) normal shock to hourly-earnings.
