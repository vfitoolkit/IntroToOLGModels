%% OLG Model A1: Solving the General Equilibrium Faster (or more Robust)
% The HeteroAgentStationaryEqm_Case1_FHorz() command solves general
% equilibrium. It does this by choosing the price(s) that minimize the
% general equilibrium conditions. By default it is using Matlab's
% fminsearch() to minimize the sum of squares of the general equilibrium
% conditions.
% This Appendix example shows to alternatives to the default. One is to set
% up a shooting algorithm, which is faster but takes a little more setup.
% The second is to use the CMA-ES algorithm which is slower but very robust
% (can find the general equilibrium in difficult situations from terrible
% initial guesses).
% Other than the following few lines, the rest of this is exactly OLG Model
% 6 (except for a tic-toc on finding the general equilibrium so if you
% run it with different settings you can see how the runtime changes). That
% is, from line 82 onward there is no difference from OLG Model 6 codes.

% The default is
% heteroagentoptions.fminalgo=1;
% To instead use CMA-ES is as simple as 
% heteroagentoptions.fminalgo=4;

% Here what we will demonstrate is how to set up a shooting algorithm to
% make finding general equilibrium faster (because we know things about how
% to minimize the general equilibrium conditions that fminsearch() is not
% able to take advantage of). The idea is that many of our general
% equilibrium conditions directly tell us if we should increase or decrease
% a certain price. For example if we set up the capital market clearance as
% "r-marginal product of capital net of depreciation", then if this is positive 
% it is because r is too big and we could update r by subtracting, say, one-tenth this 
% much from r. The way we set this up is as follows,
...
heteroagentoptions.fminalgo5.howtoupdate={...  % a row is: GEcondn, price, add, factor
    ...
    'pensions','pension',0,0.1;... % pensions GE condition will be positive if pension is too big, so subtract
    'bequests','AccidentBeq',1,0.1;... % bequests GE condition will be positive if AccidentBeq is too small, so add
    'Gtarget','G',0,0.1;... % Gtarget GE condition will be positive if G is too big, so subtract
    'govbudget','eta1',1,0.1;... % govbudget GE condition will be negative if eta1 is too big, so add
    };
...
% Just for convenience of reading and understanding these, the following commented-out lines give the general 
% equilibrium prices and conditions, copy-pasted from later in this script.
...
% Now a quick comment on other things in heteroagentoptions that might be used to make things faster or similar.

% The default of 
heteroagentoptions.multiGEcriterion=1;
% Is the sum of squares of the general equilibrium conditions. If you set
% this to zero you would instead use the sum of absolute values as the objective function.
% Related is
heteroagentoptions.multiGEweights=ones(1,5); % Five is the number of prices in this model
% The sum of squares is calculated by first taking the squares, then
% applying these weights, then summing. So you can make some general
% equilibrium conditions more (less) important by giving them higher (lower) weights.
% This is helpful if, e.g., you have numerous calibration targets set up as general 
% equilibrium conditions and you want to give these lower weights than the actual general 
% equilibrium conditions.

% Some other settings that might be of interest are (here with their default values)
...
% Note that one way to get codes faster is to loosen these (e.g. 10^(-3)). The defaults are very demanding.
% And if you want to forceably stop after a certian number of iterations (of the prices)
heteroagentoptions.maxiter=1000;

% Obviously one other way to make solving general equilibrium much faster
% is to start from a good initial guess!
...
ReturnFn=@(h,aprime,a,z,e,sigma,psi,eta,agej,Jr,J,pension,r,A,delta,alpha,kappa_j,wg1,wg2,wg3,AccidentBeq, eta1,eta2,tau)...
    OLGModel6_ReturnFn(h,aprime,a,z,e,sigma,psi,eta,agej,Jr,J,pension,r,A,delta,alpha,kappa_j,wg1,wg2,wg3,AccidentBeq, eta1,eta2,tau);
...
fprintf('With current heteroagentoptions it took %i second to solve the general equilibrium \n',GEtime)
