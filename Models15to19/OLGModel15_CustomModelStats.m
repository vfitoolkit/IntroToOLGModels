function CustomStats=OLGModel15_CustomModelStats(V,Policy,StationaryDist,Params,FnsToEvaluate,n_d,n_a,n_z,N_j,d_grid,a_grid,z_gridvals_J,pi_z_J,heteroagentoptions,vfoptions,simoptions)
...
% pi_z is the transition probabilities on (z,employment status)
% we want the transitions for idiosyncratic labor productivity
temp=sum(reshape(pi_z_J,[n_z,n_z,N_j]),4); % sum across tomorrow emp
pi_z1_J=squeeze(temp(:,1,:,:)); % for given today emp
% note: this formula assumes they are independent (they are in this model)
z1_grid=z_gridvals_J(1:n_z(1),1); % grid for idiosyncratic labor productivity
% Note: take advantage of the fact that z1_grid does not depend on j
...
for jjrev=1:Params.Jr-2
    ...
    J(:,jj)=F+(1-(Params.p_jobsep+dj-Params.p_jobsep*dj))*firmdiscountfactor*(pi_z1_J(:,:,jj)*J(:,jj+1));
end

% Take expectation of J w.r.t. the household distribution
% J is z-by-(Jr-1)
% StationaryDist is (a,z,e,j)
Distofunemployed=squeeze(sum(StationaryDist(:,:,1,1:Params.Jr-1),1)); % now: (z,j)
% StationaryDist(:,:,1,:) is unemployed, and then sum the a dimensions (for working age)
% So Distofunemployed is the distribution of the unemployed, over z and j (but only working ages)
