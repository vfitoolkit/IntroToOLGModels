%% Assignment 1: Add Government Debt to OLG Model 5
% We make five changes to the code for OLG Model 5
% First, create government debt parameter B (B for bonds). Line 82.
% Second, is that instead of FnsToEvaluate.K we have FnsToEvaluate.Assets (the total asset holdings of the households); essentially just renaming. Line 147.
% Third, is to substitute K=Assets-B into the general equilibrium condition for the capital market clearance. Line 153 (and similarly for line 156)
% Fourth, modify government budget constraint to include government expenses for the interest on the debt: r*B. Line 157.
% Fifth, some minor changes to model output in terms of what is plotted (to reflect the difference between K and Assets).
% (Note: 'A' is already being used to denote the aggregate production technology parameter.)
%
...
ReturnFn=@(h,aprime,a,agej,r,A,delta,alpha,sigma,psi,eta,Jr,pension,tau,kappa_j,J,wg1,wg2,wg3,AccidentBeq, eta1,eta2)...
    OLGModel5_ReturnFn(h,aprime,a,agej,r,A,delta,alpha,sigma,psi,eta,Jr,pension,tau,kappa_j,J,wg1,wg2,wg3,AccidentBeq, eta1,eta2);
...
FnsToEvaluate.L = @(h,aprime,a,kappa_j) kappa_j*h; % Aggregate labour supply (in efficiency units)
FnsToEvaluate.Assets = @(h,aprime,a) a; % Aggregate  physical capital
...
GeneralEqmEqns.capitalmarket = @(r,Assets,B,L,alpha,delta,A) r-alpha*A*((Assets-B)^(alpha-1))*(L^(1-alpha)); % interest rate equals marginal product of capital net of depreciation
...
GeneralEqmEqns.Gtarget = @(G,GdivYtarget,A,Assets,B,L,alpha) G-GdivYtarget*(A*(Assets-B)^(alpha)*(L^(1-alpha))); % G is equal to the target, GdivYtarget*Y
GeneralEqmEqns.govbudget = @(G,IncomeTaxRevenue,r,B) G+r*B-IncomeTaxRevenue; % Government budget balances (note that pensions are a separate budget)
...
K=AllStats.Assets.Mean-Params.B;

% GDP
Y=Params.A*(K^Params.alpha)*(AllStats.L.Mean^(1-Params.alpha));
...
fprintf('Assets-Output ratio: Assets/Y=%8.2f \n',AllStats.Assets.Mean/Y)
...
fprintf('Government-Debt-to-Output ratio: B/Y=%8.2f \n', Params.B/Y)
